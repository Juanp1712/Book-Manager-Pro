...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['csrf_token'], $submittedToken)) {
        ...
        } elseif (!preg_match('/^[\p{L}\p{N}\s\-\.,;:\'\"()!?]+$/u', $old['title'])) {
        ...
        if ($old['genre'] !== '') {
            if (mb_strlen($old['genre']) > 100) {
                $errors[] = 'El género no puede exceder 100 caracteres.';
            } elseif (!preg_match('/^[\p{L}\s\-\/]+$/u', $old['genre'])) {
        ...
        if (empty($errors)) {
            try {
                ...
                $_SESSION['flash'] = "Libro agregado exitosamente (ID: $id).";
                header('Location: index.php');
                exit;
            } catch (Exception $e) {
                error_log("Error al crear libro: " . $e->getMessage());
                $errors[] = 'Ocurrió un error al guardar el libro. Por favor, intente nuevamente.';
            }
        }
    }
